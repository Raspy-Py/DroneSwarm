...
from slam.readers import LocalReader
...
def process_frame(pts, desc):

    img = np.zeros((H, W, 3), dtype=np.uint8)
    frame = Frame(mapp, pts, desc, K)
    if frame.id == 0:
        return
...
if __name__== "__main__":
    ...
    while True:
        ...
        ret, pts, desc = reader.get_next_frame()
        if not ret and not finished:
            print("No more frames to read")
            #mapp.release()
            # When you're done and want to save the video
            #mapp.save_video('output.mp4', fps=10)
            finished = True
        # print("frame shape: ", frame.shape)
        # print("\n#################  [NEW FRAME]  #################\n")
        if ret == True and not finished:
            print("Frame: ", counter)
            counter += 1
            process_frame(pts, desc)
